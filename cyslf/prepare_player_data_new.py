...
def _translate_practice_days(row, match):
    ans = [day for day in DAY_MAP.values() if row[day] == match]

    # if row['M'] == match: ans += "M, "
    # if row['T'] == match: ans += "T, "
    # if row['W'] == match: ans += "W, "
    # if row['R'] == match: ans += "R, "
    # if row['F'] == match: ans += "F, "
...
def _load_parent_requests(filename):
    ...
    # wrong af - need to write function to translate new setup to old
    column_map = {
    ...
    # changed
    parent_reqs["preferred_days"] = parent_reqs.apply(lambda row: _translate_practice_days(row, "Preferred"), axis=1)
    
    #changed
    parent_reqs["unavailable_days"] = parent_reqs.apply(lambda row: _translate_practice_days(row, "Unavailable"), axis=1) 


    # changed
    parent_reqs["teammate_requests"] = (parent_reqs["teammate_req1_firstname"] + " " +  parent_reqs["teammate_req1_lastname"] + ", " + parent_reqs["teammate_req2_firstname"] + " " + parent_reqs["teammate_req2_lastname"])

    parent_reqs["preferred_locations"] =  parent_reqs.apply(lambda row: _translate_locations(row, "Preferred"), axis=1)

    parent_reqs["disallowed_locations"]  = parent_reqs.apply(lambda row: _translate_locations(row, "Disallowed"), axis=1) 
...
def _load_registration_data(filename):
    ...
    column_map = {
        ...
        # removed as doesn't exist in new registration data
    }
